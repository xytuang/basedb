use crate::{
    common::{constants::BASEDB_PAGE_SIZE, errors::{DiskSchedulerError, InternalError}, types::PageId},
...
pub enum DiskRequest {
    ReadWrite {
        ...
        data: Arc<Mutex<[u8; BASEDB_PAGE_SIZE]>>,
...
impl DiskScheduler {
    ...
    pub fn schedule(&self, disk_request: DiskRequest) -> Result<(), DiskSchedulerError> {
        self.sender.send(disk_request).map_err(|_| DiskSchedulerError::Internal(InternalError::ChannelError))
    }
            ...
            match disk_request {
                DiskRequest::ReadWrite {
                    ...
                    let result = if is_write {
                        disk_manager.write_page(page_id, &*buffer)
                    } else {
                        disk_manager.read_page(page_id, &mut *buffer)
                    };
...
mod tests {
    ...
    fn test_disk_scheduler() {
        let buf:Arc<Mutex<[u8; BASEDB_PAGE_SIZE]>> = Arc::new(Mutex::new([0; BASEDB_PAGE_SIZE]));
        let mut data: Arc<Mutex<[u8; BASEDB_PAGE_SIZE]>> = Arc::new(Mutex::new([0; BASEDB_PAGE_SIZE]));
        ...
        let dm = Arc::new(_dm);
        let mut disk_scheduler = DiskScheduler::new(dm);

        let (sender_1, receiver_1) = channel::<bool>();
        let (sender_2, receiver_2) = channel::<bool>();
        let disk_request_1 = DiskRequest::ReadWrite { is_write:true, data: Arc::clone(&data), page_id: 0, callback:  Some(sender_1)};
        let disk_request_2 = DiskRequest::ReadWrite { is_write:false, data: Arc::clone(&buf), page_id: 0, callback:  Some(sender_2)};
